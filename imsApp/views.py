...
def save_sales(request):
    resp = {'status':'failed', 'msg' : 'Something went wrong'}
    id = 2
    if request.method == 'POST':
        from django.db.models import Sum
        pids = request.POST.getlist('pid[]')
        for pid in pids:
            stock_count = request.POST['quantity['+str(pid)+']']
           
            
            stocks = Stock.objects.filter(product__id = pid)
            stockIn = 0 
            stockOut = 0
            for stock in stocks:
                if stock.type == '1':
            ...
            if int(stock_count)>available:
                stock_product = Product.objects.filter(id=pid).first()
                resp['msg'] = str(stock_product.name)+' has '+str(available)+ ' pieces'
                return HttpResponse(json.dumps(resp),content_type="application/json")
...
def receipt(request):
    ...
    print('id',id)
    sales = Invoice.objects.filter(id = id).first()
    print('sales',sales)
    print('quaantity',sales.item_count())
    transaction = {}
    for field in Invoice._meta.get_fields():
        if field.related_model is None:
            transaction[field.name] = getattr(Invoice,field.name)
    if 'tax_amount' in transaction:
        transaction['tax_amount'] = format(float(transaction['tax_amount']))
    print('transaction',transaction)
    ItemList = Invoice_Item.objects.filter(invoice_id = sales).all()
    context = {
        "transaction" : transaction,
        "Invoice_Item" : ItemList,
        "invoice_sales" : sales,
        'quantitly': sales.item_count(),
    }
